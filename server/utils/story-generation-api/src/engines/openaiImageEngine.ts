...
import { getOpenAIConfig, type OpenAIImageConfig } from "../config/imageGenConfig";
...
export class OpenAIImageEngine implements ImageEngine {
  async generate(input: ImageEngineInput): Promise<ImageEngineResult> {
    const { textContent, imageInputs, hasInputImages } = await partsToOpenAIInput(input.prompt_parts);
    ...
    if (process.env.TEST_MODE === "true") {
      ...
      if (input.previous_provider_ref?.response_id) {
        requestOptions.previous_response_id = input.previous_provider_ref.response_id;
      }
