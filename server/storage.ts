...
export class FirestoreStorage implements IStorage {
  ...
  // Alias for getCharacter for consistency
  ...
  async getBook(id: string): Promise<Book | undefined> {
    ...
    if (doc.exists) {
      const bookData = { id: doc.id, ...doc.data() } as Book;
      console.log(
        `[DEBUG] storage.getBook: Retrieved book ${id}, has imageJobs: ${!!bookData.imageJobs}`,
      );
      if (bookData.imageJobs) {
        console.log(
          `[DEBUG] storage.getBook: imageJobs keys: ${Object.keys(bookData.imageJobs).join(",")}`,
        );
      }
      return bookData;
    }
  ...
  async updateBookFields(
    ...
    // Special handling for imageJobs to ensure proper nested structure and deep merging
    if (fields.imageJobs && typeof fields.imageJobs === "object") {
      // Read current imageJobs, deep merge with new ones, then update
      const currentDoc = await bookRef.get();
      const currentData = currentDoc.exists ? currentDoc.data() : {};
      const currentImageJobs = currentData?.imageJobs || {};

      // Deep merge each job individually to preserve existing fields
      const mergedImageJobs = { ...currentImageJobs };

      for (const [jobId, jobUpdate] of Object.entries(fields.imageJobs)) {
        if (typeof jobUpdate === "object" && jobUpdate !== null) {
          // Merge job update with existing job data
          mergedImageJobs[jobId] = {
            ...currentImageJobs[jobId],
            ...jobUpdate,
          };
        } else {
          // Replace entirely if not an object
          mergedImageJobs[jobId] = jobUpdate;
        }
      }

      // Update with the merged imageJobs
      const updatedFields = {
        ...fields,
        imageJobs: mergedImageJobs,
      };

      await bookRef.set(updatedFields, { merge: true });
    } else {
      // Use set with merge for other fields (existing behavior)
  ...
  async getBookById(id: string): Promise<Book | null> {
    ...
    try {
      ...
      // Log raw Firestore data BEFORE any processing
      const rawData = bookDoc.data();
      console.log(
        `[DEBUG] storage.getBookById: Raw Firestore data has imageJobs: ${!!(rawData && rawData.imageJobs)}`,
      );
      if (rawData && rawData.imageJobs) {
        console.log(
          `[DEBUG] storage.getBookById: Raw imageJobs keys: ${Object.keys(rawData.imageJobs).join(",")}`,
        );
        console.log(
          `[DEBUG] storage.getBookById: Raw imageJobs type: ${typeof rawData.imageJobs}`,
        );
      }

      const bookData = { id: bookDoc.id, ...rawData } as Book;
      console.log(
        `[DEBUG] storage.getBookById: After spread, has imageJobs: ${!!bookData.imageJobs}`,
      );
      if (bookData.imageJobs) {
        console.log(
          `[DEBUG] storage.getBookById: After spread, imageJobs keys: ${Object.keys(bookData.imageJobs).join(",")}`,
        );
      }
      return bookData;
    } catch (error) {
