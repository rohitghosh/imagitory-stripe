...
export const ImageCarousel: React.FC<ImageCarouselProps> = ({
  ...
  // Enhanced touch handlers for mobile swipe with visual feedback
  const handleTouchStart = (e: React.TouchEvent) => {
    setTouchEnd(null);
    setTouchStart(e.targetTouches[0].clientX);
  ...
  return (
    <div className="relative w-full h-full bg-white flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        {/* Image with full screen coverage */}
        <div className="relative w-full h-full flex items-center justify-center">
          ...
          {/* Maximize Button - positioned directly on the image */}
          {onMaximize && !isLoading && (
            <button
              onClick={() => onMaximize(currentImage.url)}
      ...
      <div className="absolute top-4 left-1/2 transform -translate-x-1/2 px-3 py-1 bg-black/50 backdrop-blur-sm rounded-full text-white text-xs opacity-60 md:hidden">
        Swipe to navigate
      </div>
    </div>
  );
