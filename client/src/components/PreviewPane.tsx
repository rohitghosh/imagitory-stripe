...
export const PreviewPane: React.FC<PreviewPaneProps> = ({
  ...
  const navigateWithAnimation = useCallback(
    (newIndex: number, direction: "left" | "right") => {
      ...
      // Immediately reflect a loading state if the target image isn't ready
    ...
    [currentIndex, onNavigate, loadedImages],
  );

  // Preload ALL images once the list is available
  useEffect(() => {
    ...
    images.forEach((imgData, index) => {
      ...
      // Skip if we already have this index marked loaded
  ...
  // Also ensure adjacent images are attempted early (kept for responsiveness)
  useEffect(() => {
    const preloadIndices = [
      currentIndex - 1,
      currentIndex,
