...
//     try {
//       const response = await apiRequest("POST", "/api/cartoonify", {
//         characterId: "temp",
//         imageUrl: url,
//       });
//       setCartoonUrl(response.toonUrl);
//     } catch (error) {
...
//     if (!customName.trim() || !customRelation.trim() || !cartoonUrl) {
//       toast({
//         title: "Error",
    ...
    <Card className="bg-transparent border-0 shadow-none">
      <CardContent className="bg-transparent p-0">
        <div className="space-y-6">
          ...
          {/* Confirm Button restored */}
          {!(showUploadForm || uploading || cartoonPending) && (
            <div className="flex justify-center md:justify-end mt-4">
              <Button
                onClick={() => onSubmit(selectedChars)}
                className="bg-yellow-500 hover:bg-yellow-600 w-full md:w-auto"
