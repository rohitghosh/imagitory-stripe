...
export default function CreateStoryPage() {
  ...
  const [secondaryChar, setSecondaryChar] = useState<SecondaryChar | null>(
    null,
  );
  const [storyExtras, setStoryExtras] = useState<{
    moral: string;
    theme: string;
    rhyming: boolean;
  } | null>(null);
  ...
  //     kidInterests: activeCharacter.interests ?? [],
  //     storyThemes: [data.theme],
  //     characters: [data.characters[0]?.name],
  //     characterDescriptions: [data.characters[0]?.description],
  //     // reference images for hero + side character
  //     characterImageMap: {
  //       [kidName]: avatarUrlState, // hero avatar
  //       ...(data.characters[0]?.avatar
  //         ? { [data.characters[0].name]: data.characters[0].avatar }
  //         : {}),
  //     },
  //     stylePreference: bookStyle,
  //   };

  //   console.log("🚀 /api/generateFullStory payload:", payload);

  //   apiRequest("POST", "/api/generateFullStory", payload)
  //     .then((r) => {
  //       setImagesJobId(r.jobId); // use the existing progress hook
  //       setCurrentStep(3); // jump straight to preview/download
  //     })
  //     .catch((err) => log("Full-story kickoff error", err));
  // };

  const handleCustomStorySubmit = (data: {
    storyType: "custom";
    ...
    // keep the first (or only) side character
    setSecondaryChar(data.characters[0] ?? null);
    setStoryExtras({
      moral: data.moral,
      theme: data.theme,
      rhyming: data.rhyming,
    });
    toast({
      title: "Saved!",
      description: data.characters[0].name + " saved.",
    });
    // ✅  For now just show a quick popup so you can verify
    ...
    const sideChar = data.characters[0];
    const sideDescription =
      sideChar?.description ||
      // find the relation entry that points back at our hero:
      sideChar?.relations?.find(
        (r) => r.primaryCharacterId === activeCharacter.id
      )?.relation ||
      "";
    ...
    const payload = {
      ...
      kidInterests: activeCharacter.interests ?? [],
      storyThemes: [data.theme],
      characters: [sideChar?.name || ""],
      characterDescriptions: [sideDescription],
      characterImageMap: {
        [kidName]: activeCharacter.toonUrl,
        ...(sideChar?.toonUrl ? { [sideChar.name]: sideChar.toonUrl } : {}),
      },
      stylePreference: bookStyle,
    };
  ...
  //         bookId,
  //         modelId,
  //         prompt: `photo of ${activeCharacter.age}-year-old ${activeCharacter.gender} kid <${kidName}kidName> with full body visible, white background`,
  //         loraScale: newLora,
  //         stylePreference: bookStyle,
  //       }),
  //     });
      ...
      <main className="flex-grow max-w-4xl mx-auto p-6">
        ...
        {currentStep === 2 && (
          <section>
            ...
            {storyType === "predefined" ? (
              ...
              <CustomStory
                primaryCharacterId={activeCharacter?.id!}
        ...
        {currentStep === 3 && (
          <section>
            ...
            {/* <pre className="bg-gray-50 border rounded p-4 text-xs mb-6">
              {JSON.stringify(
                activeCharacter
                  ? {
                      ...
                      age: activeCharacter.age,
                      gender: activeCharacter.gender,
                      interests: activeCharacter.interests,
                      image: activeCharacter.imageUrls?.[0] ?? "",
